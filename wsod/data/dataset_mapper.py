...
class CopyAndPasteDatasetMapper(DatasetMapper):
    @configurable
    def __init__(self, is_train: bool, 
        ...
        **kwargs):
        ...
        add image labels
        """
        self.with_ann_type = False
        self.with_copy = CopyAndPaste
        if self.with_copy:
    ...
    def from_config(cls, cfg, is_train: bool = True):
        ...
        ret.update({
            ...
            'CopyAndPaste': cfg.WITH_COPY_PASTE,
        })

        if ret['CopyAndPaste'] and is_train:
            ret['Image_json'] = cfg.IMAGE_JSON_PATH
    ...
    def __call__(self, dataset_dict:dict, imageNet_dict: list) -> dict:
        ...
        if self.with_copy:
            with open(self.image_json_path, 'r') as f:
                copy_info = json.load(f)
                f.close()
            # ---------------------------------- #
            # TODO
            # a way to sample image infomation
            # ---------------------------------- #
        ...
        if len(ImageNet_dict) != 0:
            ...
            append_cls = np.asarray(append_cls)
        
        # final info
        image_shape = new_image.shape[1:]
        # get Instaces
        if len(new_bbox) != 0 and len(append_box) != 0:
            bbox = np.concatenate(append_box, new_bbox)
            category = np.concatenate(append_cls, ins_cls)
        elif len(new_bbox) == 0 and len(append_box) != 0:
            bbox = append_box
            category = append_cls
        elif len(new_bbox) != 0 and len(append_box) == 0:
            bbox = new_bbox
    ...
    def _remove_small_regions(self, segs:np.ndarray, threshhod=300)-> np.array:

        mask = morphology.remove_small_objects(segs, threshhod)
        mask = mask.sum(axis = 1).sum(axis = 1) > 0
